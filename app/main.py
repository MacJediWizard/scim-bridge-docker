...
async def replace_user(user_id: str, user: SCIMUser, authorization: str = Header(None)):
    if authorization != f"Bearer {SCIM_TOKEN}":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    # Ensure mailbox exists in Mailcow during full-sync
    status_code, text = await create_mailcow_mailbox(user_id, user.name.get("formatted", user_id))
    if status_code != 200 and status_code != 409:
        # 409 may indicate already exists; ignore
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Mailcow error: {text}")
    return {
        "schemas": ["urn:ietf:params:scim:schemas:core:2.0:User"],
        "id": user_id,
        "userName": user_id,
...
async def create_user(user: SCIMUser, authorization: str = Header(None)):
    if authorization != f"Bearer {SCIM_TOKEN}":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    status_code, text = await create_mailcow_mailbox(user.userName, user.name.get("formatted", user.userName))
    if status_code != 200:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Mailcow error: {text}")
    return {
        "schemas": ["urn:ietf:params:scim:schemas:core:2.0:User"],
